...
- Tabs L→R: **5/new** (up to 5 unseen pool words/day), **5/review** (5 flashcards/day then unlimited), **Translate**, **Progress** (bundled frequency lists + unlock highlighting). **Settings** is a header cog, not a tab. UI strings: `en` / `pt-BR` (Brazilian, not European) in `i18n.js`. Pre-login: `navigator.languages`. Signed-in override: `users.app_lang` (`NULL` = keep detecting). `<html lang>` follows app language. Translate / Progress inline translate use app language as the non-learning pole (fallback `EN` when it equals the learning language). Tab labels pt-BR: `5/novas`, `5/revisar`, `Progresso`.
- Daily: `WORDS_PER_DAY = 5` (`daily-pool.js`), `DAILY_REVIEW_GOAL = 5` (`ten-logic.js`). Both 5/new and 5/review show the same bottom counter: unique new words seen (up to 5) plus reviews graded (up to 5), out of 10. Confetti once per language per calendar day only after both the 5th new word has been reached and the 5th review graded, in either order (`localStorage` gates). The celebration marker is saved only when confetti starts; an unavailable script or Reduce Motion leaves it unconsumed. Next stays enabled on the last new word and opens Review. After 5 reviews, **∞** — keep going. `resolveStartupTab`: 5/new unless that day's new-word completion marker is set → 5/review unless today's 5 grades are done → Translate.
...
## Client behavior
- **5/new:** Pick up to 5 pool words not yet surfaced (viewed on a daily card or unlocked via single-word translate / Progress inline translate). Unlocked words never appear even if still in the pool. Saved assignment persisted (`/api/daily-words`); refresh keeps the same list. Unviewed words from today are not marked surfaced and return to the pool. Assigned headwords already in today's list stay after they are surfaced in 5/new — the seen set blocks **new** picks/refills only. Words unlocked *outside* 5/new while still on today's list are dropped live (`reconcileTodayWordsAfterUnlock`) and refilled. Assignment longer than 5 is truncated to the first 5 on load. Card index via `/api/daily-progress`. Glosses: one `/api/daily-glosses/ensure` on boot (any tab) into `daily_word_glosses`; English JSON fields when app language is `en` (no paid call). Example sentences sit in a nested dropdown labeled **new word used in context**, collapsed by default; each open reveals one more sentence (up to 2; the third example stays in the pool JSON). Native-language context translations start hidden; clicking the learning-language sentence reveals its saved translation without a translation API call or instructional text. Translations fade and slide in when revealed (disabled for reduced-motion preferences). Changing cards hides both translations again. **Add all to review** adds the word and only those first two examples. Nested **another** labels hide after open and cannot be collapsed. Chevron is a right-facing triangle that rotates down on open. Newly revealed sentences fade and slide in on every open, including reopen after close. Footer pool-days: unseen pool words / 5, dev only.
- **5/review:** Dots toward 5; then ∞. Queue from `/api/cards/queue`. Grade buttons stay enabled except while editing a card (do not `disabled` on `reviewSubmitting`). After async grade/delete/edit, clear busy flag and `renderReview()` in `finally`. Daily grade count is client `localStorage`, not SQLite.
...
- **i18n wiring:** `index.html` English is first-paint fallback. After boot, `applyAppLanguage()` overwrites `[data-i18n]`, placeholders, titles, aria-labels from `i18n.js`. `app.js?v=` and `styles.css?v=` URLs in `index.html` must be bumped when those assets change with the page: the installed PWA has reused older JavaScript after an HTML update, crashing on removed element IDs.
