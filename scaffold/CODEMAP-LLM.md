...
- Daily: `WORDS_PER_DAY = 5` (`daily-pool.js`), `DAILY_REVIEW_GOAL = 5` (`ten-logic.js`). Confetti once per language per calendar day only after both the 5th new word has been reached and the 5th review graded, in either order (`localStorage` gates). Next stays enabled on the last new word and opens Review. After 5 reviews, **∞** — keep going. `resolveStartupTab`: 5/new unless that day's new-word completion marker is set → 5/review unless today's 5 grades are done → Translate.
